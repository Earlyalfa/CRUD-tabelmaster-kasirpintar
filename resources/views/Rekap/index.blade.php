...
<head>
    ...
    <title>Rekapitulasi Stok Bulanan</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
</head>
<body>
<div class="container mt-5">
    <h1 class="mb-4">Rekapitulasi Stok Bulanan</h1>
        ...
        <tbody>
            @foreach($rekap as $item)
                <tr>
                    ...
                    <td>{{ $item->total_rusak_hilang }}</td>
                    <td>{{ $item->stok_akhir }}</td>
                    <td>
                        <a href="{{ route('rekap.edit', $item->id) }}" class="btn btn-warning btn-sm">Edit</a>
                        <form action="{{ route('rekap.destroy', $item->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
